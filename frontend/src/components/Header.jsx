...
import Hero from "../img/CarrouselNav/hero.jpg";
import Slider2 from "../img/CarrouselNav/slider2.jpg";
import Slider3 from "../img/CarrouselNav/slider3.jpg";
    ...
    try {
      if (localStorage.getItem("token")) {
        const usuario = await clienteAxios.get(
          `/api/v1/usuarios/${localStorage.getItem("id")}`
        );
  ...
  const alturaNav = () => {
    ...
    if (pathHome) {
      ...
      if (carrouselAlt) {
        if (
          window.pageYOffset <=
          carrouselAlt - (window.screen.width > 767 ? 80 : 160)
        ) {
    ...
    <div>
      ...
      >
        {roleAdmin === "admin" ? (
          ...
          <Navbar
            variant="dark"
            className="px-0 py-2 mb-3 mb-lg-4 navbar-menu row flex-wrap justify-content-center justify-content-md-between m-0"
          >
            <Nav className="d-none d-sm-none d-md-flex d-lg-flex row pl-5 order-2 order-md-1">
              <Navbar.Brand className="d-none d-lg-block ml-3">
                <img
                  className=""
                  style={{ width: "40px" }}
                  src={Logo}
              ...
              <Link className="text-white hover-navbar mt-2 mx-1" to="/">
                INICIO
              </Link>
              <Link
                className="text-white hover-navbar mt-2 mx-1"
                to="/"
                onClick={Timeout}
              >
                FAVORITOS
              ...
              <Link
                className="text-white hover-navbar mt-2 mx-1"
                to="#AboutUs"
                onClick={() => setSobreShow(true)}
              >
                SOBRE NOSOTROS
              </Link>
              <SobreNosotros
                show={sobreShow}
                onHide={() => setSobreShow(false)}
              />
            </Nav>
            <Nav className="row mx-2 order-1 order-md-2 flex-nowrap">
              {auth.isAuthenticated() ? (
                <>
                  <label
                    className="mt-2 text-white hover-navbar dropdown-toggle"
                    onClick={MoverMenuUser}
                  ...
                  <div className="dropdown">
                    ...
                    >
                      <Link
                        className="dropdown-item text-white hover-navbar"
                        to="/user/orders"
                      >
                        <i className="far fa-clipboard"></i> Mis Favoritos
                      </Link>
                      <Link
                        className="dropdown-item text-white hover-navbar"
                        ref={menuUser}
                        to={`/user/perfil/${localStorage.getItem("id")}`}
                      >
                        <i className="far fa-user"></i> Mis datos
                      </Link>
                    </div>
                  </div>
                  <Nav.Link
                    className="text-white hover-navbar"
                ...
                <>
                  <Link className="text-white hover-navbar mx-2" to="/reg">
                    REGISTRO
                  </Link>
                  <Link
                    className="text-white hover-navbar mx-2"
                    to=""
            ...
            <Form className="w-100 px-3 d-md-none mt-3 mb-0">
              <Form.Group>
                <Form.Control
                  ...
                  defaultValue={0}
                  ref={formSelect}
                  className="drop-menu text-white"
                  as="select"
                  custom
                >
                  <option disabled>IR A...</option>
                  <option value="/">INICIO</option>
                  <option value="#Menu">FAVORITOS</option>
